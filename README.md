<!-- Banner (optional) -->
<!-- ![Banner](https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_USERNAME/main/banner.png) -->

<h1 align="center">Hi 👋, I'm Krishna Reddy</h1>
<h3 align="center">Java Full Stack Developer | Spring Boot | Angular</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/ksaikrishnareddy/">LinkedIn</a> •
  <a href="https://leetcode.com/YOUR_LEETCODE/">LeetCode</a> •
  <a href="mailto:saikrishnakothakapu@gmail.com">Email</a>
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?lines=Java+Full+Stack+Developer;Spring+Boot+%7C+Angular;REST+APIs+%7C+Microservices;DSA+Learner;Open+to+Opportunities&center=true&width=600&height=45">
</p>

---

## 👨‍💻 About Me
- 💼 Software Engineer at **Infosys**
- 💻 Java Full Stack Developer (Spring Boot + Angular)
- 📦 Experience building REST APIs & full-stack applications
- 📚 Strong foundation in **DSA & OOP**
- 🎯 Goal: Crack **product-based companies (10+ LPA)**

---

## 🛠️ Tech Stack

<p>
  <img src="https://skillicons.dev/icons?i=java,spring,angular,html,css,js,mysql,git,github" />
</p>

---

## 🚀 Projects
- **Insurance Management System**  
  Java, Spring Boot, Angular, MySQL  
  Secure login, policy purchase & management, agent referrals

- **Appointment Booking System**  
  Spring Boot, JPA, REST APIs  
  Doctor availability, time-slot generation, validations

- **Goal Tracker Application**  
  Java, Spring Boot  
  User-specific goals, CRUD APIs, clean service architecture

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=radical" />
</p>

---

## 📌 What I'm Currently Working On
- Improving **DSA problem-solving**
- Building **real-world backend projects**
- Preparing for **Java Full Stack interviews**

---

## 🤝 Let's Connect
- LinkedIn: https://www.linkedin.com/in/YOUR_LINKEDIN/
- Email: YOUR_EMAIL@gmail.com

---

⭐ **If you like my work, consider starring my repositories!**
