<!-- ===================== BANNER (OPTIONAL) ===================== -->
<!-- ![Banner](https://raw.githubusercontent.com/saikrishnakothakapu/saikrishnakothakapu/main/banner.png) -->

<h1 align="center">Hi 👋, I'm SaiKrishna Reddy Kothakapu</h1>
<h3 align="center">Java Full Stack Developer | Spring Boot | Angular</h3>

<!-- ===================== TYPING ANIMATION ===================== -->
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?lines=Java+Full+Stack+Developer;Spring+Boot+%7C+Angular;REST+APIs+%7C+Microservices;DSA+%7C+Problem+Solving;Open+to+Opportunities&center=true&width=650&height=45&v=1" />
</p>

<!-- ===================== SOCIAL ICONS ===================== -->
<p align="center">
  <a href="https://www.linkedin.com/in/ksaikrishnareddy/" target="_blank" rel="noopener noreferrer">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" width="35" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://leetcode.com/u/saikrishnareddyK11/" target="_blank" rel="noopener noreferrer">
    <img src="https://upload.wikimedia.org/wikipedia/commons/1/19/LeetCode_logo_black.png" width="35" />
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="mailto:saikrishnakothakapu@gmail.com">
    <img src="https://img.icons8.com/color/48/gmail-new.png" width="35" />
  </a>
</p>

<!-- ===================== PROFILE VIEWS ===================== -->
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=saikrishnakothakapu&label=Profile%20Views&color=blue&style=flat" />
</p>

---

## 👨‍💻 About Me
- 💼 Software Engineer at **Infosys**
- 💻 Java Full Stack Developer (Spring Boot + Angular)
- 📦 Experience building REST APIs & backend services
- 📚 Strong foundation in **DSA & OOP**
- 🎯 Goal: Crack **product-based companies (10+ LPA)**

---

## 🛠️ Tech Stack
<p>
  <img src="https://skillicons.dev/icons?i=java,spring,angular,html,css,js,mysql,git,github" />
</p>

---

## 🚀 Projects
- **Insurance Management System**  
  Java, Spring Boot, Angular, MySQL  
  Secure login, policy purchase & agent referrals

- **Appointment Booking System**  
  Spring Boot, JPA, REST APIs  
  Doctor availability, time-slot generation

- **Goal Tracker Application**  
  Java, Spring Boot  
  CRUD APIs with clean service architecture

---

## 📊 GitHub & LeetCode Stats

<!-- ===================== 3 STATS SIDE BY SIDE ===================== -->
<p align="center">
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=saikrishnakothakapu&theme=radical"
    width="32%"
  />
  <img
    src="https://streak-stats.demolab.com/?user=saikrishnakothakapu&theme=radical"
    width="32%"
  />
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=saikrishnakothakapu&theme=radical"
    width="32%"
  />
</p>

<!-- ===================== LEETCODE STATS ===================== -->
<p align="center">
  <img
    src="https://leetcode-stats-six.vercel.app/api?username=saikrishnareddyk11&theme=dark"
    width="45%"
  />
</p>

---

## 📌 Currently Working On
- Improving **DSA problem-solving**
- Building **real-world Spring Boot projects**
- Preparing for **Java Full Stack interviews**

---

<!-- ===================== CONTRIBUTION SNAKE (OPTIONAL ADVANCED) ===================== -->

---

## 🐍 Contribution Graph

![Contribution Snake](https://raw.githubusercontent.com/saikrishnakothakapu/saikrishnakothakapu/output/github-contribution-grid-snake.svg)

---


---

⭐ **If you like my work, consider starring my repositories!**
